...
torch_losses = [nn.BCELoss(), nn.BCEWithLogitsLoss()]
...
class TestTorchPatchPerformance:
    ...
    def test_measure_setter_valid(self, measure, loss):
        patch_performance = TorchPatchPerformance(loss=loss, measure=measure)
    ...
    def test_measure_setter_invalid(self, loss):
        with pytest.raises(ValueError):
            TorchPatchPerformance(loss=loss, measure=invalid_measure)

    @pytest.mark.parametrize("measure", valid_measures)
    @pytest.mark.parametrize("loss", torch_losses)
    def test_track_valid(self, measure, loss):
        patch_performance = TorchPatchPerformance.track(loss=loss, measure=measure)
        assert patch_performance.measure == measure
        assert patch_performance.loss == loss
    ...
    def test_track_invalid(self, loss):
        with pytest.raises(ValueError):
            TorchPatchPerformance.track(loss=loss, measure=invalid_measure)

    @pytest.mark.parametrize("measure", valid_measures)
    @pytest.mark.parametrize("loss", torch_losses)
    def test_call(self, measure, loss, dummy_torch_predictions, dummy_torch_targets):
        patch_performance = TorchPatchPerformance.track(loss=loss, measure=measure)
    ...
    @pytest.mark.parametrize("measure", valid_measures)
    @pytest.mark.parametrize("loss", torch_losses)
    def test_simple_training(self, measure, loss, dummy_torch_targets):
        dummy_torch_targets = torch.from_numpy(dummy_torch_targets)
        model = nn.Sequential(nn.Conv2d(1, 1, (3, 3), padding=1), nn.Sigmoid())
